...
def read_data(data_dir, dataset_name, iteration):
    ...
    y_pool.index = pd.RangeIndex(start=len(y_train), stop=len(y_train) + len(y_pool), step=1)
    y_pool_nan.index = pd.RangeIndex(start=len(y_train), stop=len(y_train) + len(y_pool), step=1)

    return X_train, y_train, X_pool, y_pool, y_pool_nan, X_rest, y_rest, X_test, y_test, target_length, target_names, feature_names
