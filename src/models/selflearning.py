...
class TargetSelfLearning(SelfLearning):
        ...
        def training(self, X_train_df, y_train_df, X_pool_df, X_test,  y_test, target_length, feature_names, fold_index, y_pool):
            ...
            return added_pairs_per_iteration, X_train, X_pool, y_train_df, X_test, y_test,  target_length, feature_names, fold_index, y_pool
        ...
        def train_and_evaluate(self, fold_index, X_train, y_train_df, X_pool, X_test, y_test, y_pool, target_length, target_names, feature_names):
            ...
            added_pairs_per_iteration, X_train, X_pool, y_train_df, X_test, y_test,  target_length, feature_names, fold_index, y_pool = self.training(
                X_train, y_train_df, X_pool, X_test, y_test, target_length, feature_names, fold_index, y_pool
            )
            return self.R2, self.MSE, self.MAE, self.CA, self.ARRMSE, added_pairs_per_iteration, fold_index

if __name__ == "__main__":
    data_dir = config.DATA_DIR
    dataset_name = config.DATASET_NAME
    
    print('Target-based SelfLearning...')
    ...
    for i in range(k_folds):
        ...
        R2, MSE, MAE, CA, ARRMSE, added_pairs_per_iteration,fold_index = target_selflearning_model.train_and_evaluate(
