...
for i in range(Method.iterations):
    ...
    plt.plot(proposed_method_instance.epochs, proposed_method_instance.instance_CA[i,:-1],'b', label='Instance based QBC')
    plt.plot(proposed_method_instance.epochs, upperbound_method.upperbound_CA[i,:-1],'r', label='Upper bound')
    plt.plot(proposed_method_instance.epochs, lowerbound_method.random_CA[i,:-1],'g', label='Random sampling')
    plt.plot(proposed_method_instance.epochs, baseline_method.baseline_CA[i,:-1],'y', label='Greedy sampling')
    plt.legend(loc='best')
    plt.xlabel('epoch')
    plt.ylabel('CA')
    plt.title('Instance based QBC method CA performance for {} dataset: iteration {}'.format(Method.dataset_name, i+1))
    ca_fig_path = fig_path / 'ca' / f'ca_{i+1}'
    plt.savefig(ca_fig_path)
    plt.show()

instance_mean_R2 =  np.mean(proposed_method_instance.instance_R2, axis=0)
instance_mean_MSE =  np.mean(proposed_method_instance.instance_MSE, axis=0)
instance_mean_MAE =  np.mean(proposed_method_instance.instance_MAE, axis=0)
instance_mean_CA =  np.mean(proposed_method_instance.instance_CA, axis=0)

upperbound_mean_R2 =  np.mean(upperbound_method.upperbound_R2, axis=0)
upperbound_mean_MSE =  np.mean(upperbound_method.upperbound_MSE, axis=0)
upperbound_mean_MAE =  np.mean(upperbound_method.upperbound_MAE, axis=0)
upperbound_mean_CA =  np.mean(upperbound_method.upperbound_CA, axis=0)

random_mean_R2 =  np.mean(lowerbound_method.random_R2, axis=0)
random_mean_MSE =  np.mean(lowerbound_method.random_MSE, axis=0)
random_mean_MAE =  np.mean(lowerbound_method.random_MAE, axis=0)
random_mean_CA =  np.mean(lowerbound_method.random_CA, axis=0)

greedy_mean_R2 =  np.mean(baseline_method.baseline_R2, axis=0)
greedy_mean_MSE =  np.mean(baseline_method.baseline_MSE, axis=0)
greedy_mean_MAE =  np.mean(baseline_method.baseline_MAE, axis=0)
greedy_mean_CA =  np.mean(baseline_method.baseline_CA, axis=0)
...
plt.plot(proposed_method_instance.epochs, instance_mean_CA[:-1],'b', label='Instance based QBC')
plt.plot(proposed_method_instance.epochs, upperbound_mean_CA[:-1],'r', label='Upper bound')
plt.plot(proposed_method_instance.epochs, random_mean_CA[:-1],'g', label='Random sampling')
plt.plot(proposed_method_instance.epochs, greedy_mean_CA[:-1],'y', label='Greedy sampling')
plt.legend(loc='best')
plt.xlabel('epoch')
plt.ylabel('CA')
plt.title('Instance based QBC method CA performance for {} dataset'.format(Method.dataset_name))
ca_all_fig_path = fig_path / 'ca' 
plt.savefig(ca_all_fig_path)
plt.show()

# Append the average performance to the iteration performances
instance_total_R2 = np.append(proposed_method_instance.instance_R2, np.reshape(instance_mean_R2, (1,n_epochs+1)), axis=0)
instance_total_MSE = np.append(proposed_method_instance.instance_MSE, np.reshape(instance_mean_MSE, (1,n_epochs+1)), axis=0)
instance_total_MAE = np.append(proposed_method_instance.instance_MAE, np.reshape(instance_mean_MAE, (1,n_epochs+1)), axis=0)
instance_total_CA = np.append(proposed_method_instance.instance_CA, np.reshape(instance_mean_CA, (1,n_epochs+1)), axis=0)

upperbound_total_R2 = np.append(upperbound_method.upperbound_R2, np.reshape(upperbound_mean_R2, (1,n_epochs+1)), axis=0)
upperbound_total_MSE = np.append(upperbound_method.upperbound_MSE, np.reshape(upperbound_mean_MSE, (1,n_epochs+1)), axis=0)
upperbound_total_MAE = np.append(upperbound_method.upperbound_MAE, np.reshape(upperbound_mean_MAE, (1,n_epochs+1)), axis=0)
upperbound_total_CA = np.append(upperbound_method.upperbound_CA, np.reshape(upperbound_mean_CA, (1,n_epochs+1)), axis=0)

random_total_R2 = np.append(lowerbound_method.random_R2, np.reshape(random_mean_R2, (1,n_epochs+1)), axis=0)
random_total_MSE = np.append(lowerbound_method.random_MSE, np.reshape(random_mean_MSE, (1,n_epochs+1)), axis=0)
random_total_MAE = np.append(lowerbound_method.random_MAE, np.reshape(random_mean_MAE, (1,n_epochs+1)), axis=0)
random_total_CA = np.append(lowerbound_method.random_CA, np.reshape(random_mean_CA, (1,n_epochs+1)), axis=0)

greedy_total_R2 = np.append(baseline_method.baseline_R2, np.reshape(greedy_mean_R2, (1,n_epochs+1)), axis=0)
greedy_total_MSE = np.append(baseline_method.baseline_MSE, np.reshape(greedy_mean_MSE, (1,n_epochs+1)), axis=0)
greedy_total_MAE = np.append(baseline_method.baseline_MAE, np.reshape(greedy_mean_MAE, (1,n_epochs+1)), axis=0)
greedy_total_CA = np.append(baseline_method.baseline_CA, np.reshape(greedy_mean_CA, (1,n_epochs+1)), axis=0)
...
instance_df_R2 = pd.DataFrame(instance_total_R2, index=rows, columns=cols)
instance_df_MSE = pd.DataFrame(instance_total_MSE, index=rows, columns=cols)
instance_df_MAE = pd.DataFrame(instance_total_MAE, index=rows, columns=cols)
instance_df_CA = pd.DataFrame(instance_total_CA, index=rows, columns=cols)

upperbound_df_R2 = pd.DataFrame(upperbound_total_R2, index=rows, columns=cols)
upperbound_df_MSE = pd.DataFrame(upperbound_total_MSE, index=rows, columns=cols)
upperbound_df_MAE = pd.DataFrame(upperbound_total_MAE, index=rows, columns=cols)
upperbound_df_CA = pd.DataFrame(upperbound_total_CA, index=rows, columns=cols)

random_df_R2 = pd.DataFrame(random_total_R2, index=rows, columns=cols)
random_df_MSE = pd.DataFrame(random_total_MSE, index=rows, columns=cols)
random_df_MAE = pd.DataFrame(random_total_MAE, index=rows, columns=cols)
random_df_CA = pd.DataFrame(random_total_CA, index=rows, columns=cols)

greedy_df_R2 = pd.DataFrame(greedy_total_R2, index=rows, columns=cols)
greedy_df_MSE = pd.DataFrame(greedy_total_MSE, index=rows, columns=cols)
greedy_df_MAE = pd.DataFrame(greedy_total_MAE, index=rows, columns=cols)
greedy_df_CA = pd.DataFrame(greedy_total_CA, index=rows, columns=cols)
...
instance_df_CA.to_csv(folder_path / 'ca' / 'instance_ca.csv' , header=cols) 
...
upperbound_df_CA.to_csv(folder_path / 'ca' / 'upperbound_ca.csv' , header=cols)
...
random_df_CA.to_csv(folder_path / 'ca' / 'random_ca.csv' , header=cols)
...
greedy_df_CA.to_csv(folder_path / 'ca' / 'greedy_ca.csv' , header=cols)
